"""
Demo09.3 - MouseControl.dll + 简单比例控制（无PID）
直接按比例移动鼠标，简单快速
"""
...
def load_mouse_driver():
    ...
    dll_path = os.path.join(script_dir, "MouseControl.dll")
...
AUTO_CLICK = True
CLICK_INTERVAL = 0.1       # 开火间隔（秒）
TARGET_PRIORITY = 'nearest'
SHOW_OVERLAY = True

# 简单控制参数
AIM_SPEED = 1.0        # 瞄准速度：1.0=一次到位，0.5=移动一半距离
AIM_MAX_MOVE = 150     # 单次最大移动像素，防止瞬移
AIM_MIN_MOVE = 1       # 最小移动像素，小于此值不移动
SMOOTHING = 0.0        # 平滑系数：0=不平滑，0.5=中等平滑
...
def overlay_process(overlay_queue, running_event, win_x, win_y, win_w, win_h):
    ...
    while running_event.is_set():
        ...
        cv2.putText(overlay, f"Speed:{AIM_SPEED} R:{click_radius}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
        if conf > 0:
...
if __name__ == '__main__':
    ...
    print("=" * 40)
    print("Demo09.3 - Simple Proportional Control (No PID)")
    print(f"Speed: {AIM_SPEED}, MaxMove: {AIM_MAX_MOVE}")
    ...
    # 平滑变量
    smooth_move_x, smooth_move_y = 0, 0
    ...
    while running_event.is_set():
        ...
        if AIM_ENABLED and all_boxes and aim_active:
            ...
            if target:
                ...
                # 简单比例移动（无PID）
                if current_dist >= AIM_MIN_MOVE:
                    # 计算移动量
                    move_x = error_x * AIM_SPEED
                    move_y = error_y * AIM_SPEED

                    # 限制最大移动
                    move_x = max(-AIM_MAX_MOVE, min(AIM_MAX_MOVE, move_x))
                    move_y = max(-AIM_MAX_MOVE, min(AIM_MAX_MOVE, move_y))

                    # 可选平滑
                    if SMOOTHING > 0:
                        smooth_move_x = SMOOTHING * smooth_move_x + (1 - SMOOTHING) * move_x
                        smooth_move_y = SMOOTHING * smooth_move_y + (1 - SMOOTHING) * move_y
                        move_x, move_y = smooth_move_x, smooth_move_y
