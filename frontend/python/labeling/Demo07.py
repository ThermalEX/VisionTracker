...
if __name__ == '__main__':
    ...
    model = YOLO("../models/best 12.12.pt")
    ...
    while running_event.is_set():
        ...
        status_text = f"FPS: {current_fps:.1f} | PyTorch | ROI:{crop_w}x{crop_h}"
        ...
        if SHOW_PERF:
            perf_text = f"C:{perf_stats['crop']*1000:.1f} I:{perf_stats['inference']*1000:.1f} D:{perf_stats['draw']*1000:.1f} T:{perf_stats['total']*1000:.1f}ms"
            cv2.putText(overlay,
                        perf_text,
                        (w - 280, 30),
                        cv2.FONT_HERSHEY_SIMPLEX,
