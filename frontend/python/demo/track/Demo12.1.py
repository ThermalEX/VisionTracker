"""
Demo12.1 - Pure PID tracking (no snap)
纯PID控制器 - 用于与Demo12.2 ADRC做对比录像
...
# ==================== PID Controller ====================
class PIDController:
    def __init__(self, kp=0.3, ki=0.0, kd=0.001, max_output=200, integral_limit=100):
        self.kp = kp
        self.ki = ki
        self.kd = kd
    ...
    def compute(self, error_x, error_y):
        ...
        self.integral_x = max(-self.integral_limit, min(self.integral_limit, self.integral_x + error_x))
        self.integral_y = max(-self.integral_limit, min(self.integral_limit, self.integral_y + error_y))
...
def overlay_process(overlay_queue, running_event, win_x, win_y, win_w, win_h):
    ...
    while running_event.is_set():
        ...
        # FOV corners
        corner_len = 30
        cv2.line(overlay, (x1_roi, y1_roi), (x1_roi + corner_len, y1_roi), (255, 255, 255), 1)
        cv2.line(overlay, (x1_roi, y1_roi), (x1_roi, y1_roi + corner_len), (255, 255, 255), 1)
        cv2.line(overlay, (x2_roi, y1_roi), (x2_roi - corner_len, y1_roi), (255, 255, 255), 1)
        cv2.line(overlay, (x2_roi, y1_roi), (x2_roi, y1_roi + corner_len), (255, 255, 255), 1)
        cv2.line(overlay, (x1_roi, y2_roi), (x1_roi + corner_len, y2_roi), (255, 255, 255), 1)
        cv2.line(overlay, (x1_roi, y2_roi), (x1_roi, y2_roi - corner_len), (255, 255, 255), 1)
        cv2.line(overlay, (x2_roi, y2_roi), (x2_roi - corner_len, y2_roi), (255, 255, 255), 1)
        cv2.line(overlay, (x2_roi, y2_roi), (x2_roi, y2_roi - corner_len), (255, 255, 255), 1)
        ...
        if head_x is not None:
            # Target direction line (yellow for PID)
            dx = head_x - center_x
            dy = head_y - center_y
        ...
        cv2.putText(overlay, f"AIM:{'ON' if aim_active else 'OFF'}", (10, 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, aim_color, 2)
        cv2.putText(overlay, f"FIRE:{'ON' if AUTO_CLICK else 'OFF'}", (120, 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        cv2.putText(overlay, "Mode:PID", (10, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
        cv2.putText(overlay, f"kp:{kp:.2f} ki:{KI:.3f} kd:{KD:.3f}", (10, 75),
...
def calibrate(model, use_trt, frame_queue, mouse, pid, overlay_queue):
    """Calibrate PID KP using one-shot move method."""
    global KP

    CALIBRATE_SAMPLES = 3
    print("\n" + "=" * 50)
    print("开始校准 KP - 请将准心对准一个静止目标")
    print(f"当前 KP = {pid.kp:.3f}")
    print("=" * 50)

    time.sleep(0.5)

    for iteration in range(CALIBRATE_SAMPLES):
        print(f"\n--- 迭代 {iteration + 1}/{CALIBRATE_SAMPLES} (KP={pid.kp:.3f}) ---")
        ...
        print(f"  目标: ({target_x}, {target_y}), 误差: ({error_x:.0f}, {error_y:.0f}), 距离: {error_dist:.1f}px")
        ...
        # One-shot move: kp * error should bring crosshair to target
        move_x = int(pid.kp * error_x)
        move_y = int(pid.kp * error_y)
        ...
        if abs(actual_move_x) > 5:
            ...
            pid.kp = max(0.05, min(5.0, pid.kp * adjust))
            print(f"  调整系数: {adjust:.3f}, 新 KP: {pid.kp:.3f}")
    ...
    # Calibrated KP is for one-shot moves; continuous PID needs a smaller gain
    damping = 3.0
    pid.kp = max(0.05, min(5.0, pid.kp / damping))
    KP = pid.kp
    print("\n" + "=" * 50)
    print(f"校准完成！KP = {pid.kp:.3f} (已除以 {damping} 衰减)")
    print("=" * 50 + "\n")
...
if __name__ == "__main__":
    ...
    for pt_file in glob.glob(os.path.join(models_dir, "*.pt")):
        all_models.append({"name": os.path.basename(pt_file), "path": pt_file, "type": "pt"})
    for engine_file in glob.glob(os.path.join(models_dir, "*.engine")):
        all_models.append({"name": os.path.basename(engine_file), "path": engine_file, "type": "engine"})
    ...
    print("=" * 50)
    print("Demo12.1 - Pure PID")
    print(f"kp={KP}, ki={KI}, kd={KD}")
    print("-" * 50)
    print("F6       = 自动校准 KP（对准静止目标后按）")
    ...
    while running_event.is_set():
        ...
        if AIM_ENABLED and all_boxes and aim_active:
            ...
            if target:
                ...
                # PID control
                u_x, u_y = pid.compute(error_x, error_y)
